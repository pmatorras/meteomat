def execute(args):
    ...
    if args.era5:
        ...
        download_all_regions_parallel(year=year, max_workers=5, force_refresh=args.force_refresh)
    if args.aemet:
        print("Fetching AEMET stations")
        from meteomat.datasets.fetch_stations import filter_stations_by_regions
        from meteomat.datasets.download_aemet_data import download_aemet
        regions_aemet = filter_stations_by_regions(force_refresh=args.force_refresh)
        if args.verbose:
            # Show examples
        ...
        download_aemet(sel_regions=args.regions, force_refresh=args.force_refresh)